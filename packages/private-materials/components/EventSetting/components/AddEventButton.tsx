  ...
  setup(props, { emit }) {
    ...
    return () => (
      ...
      >
        {{
          tooltip: () => (
            <div class="c-text-black c-py-1 dark:c-text-white dark:c-bg-darkMode">
              {props.eventList.map((eventItem, index) => {
                return (
                  <div>
                    {/* 如果不是第一个 可以来一个分割线 */}
                    {index !== 0 && (
                      <div class="c-h-[1px] c-w-full c-bg-gray-200 dark:c-border-darkMode c-my-1" />
                    )}
                    ...
                    {eventItem.events.map((event) => {
                      return (
                        <div
                          ...
                          class="c-h-8 c-pl-2 c-flex c-justify-start c-items-center c-cursor-pointer hover:c-bg-gray-200 dark:hover:c-bg-gray-800"
                        >
                          {event.describe}
                        </div>
                      );
