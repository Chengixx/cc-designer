...
export interface Props {
  ...
  tooltip?: string | JSX.Element; // 文字提示内容 string | slot
...
useResizeObserver(
  [tooltipCardRef, contentRef],
  (entries: readonly ResizeObserverEntry[]) => {
    // 排除 tooltip-card 显示过渡动画时的尺寸变化
    if (
      entries.length === 1 &&
      entries[0].target.className === "tooltip-card"
    ) {
      const { blockSize, inlineSize } = entries[0].borderBoxSize[0];
      if (
        blockSize === tooltipCardHeight.value &&
        inlineSize === tooltipCardWidth.value
      ) {
        return;
      }
    }
...
function getPlacement(): string {
  ...
  const {
    ...
    left: targetLeft,
    right: targetRight,
  } = getShelterRect(); // 滚动元素或视口各边缘相对于浏览器视口的位置(不包括滚动条)
  ...
  function findPlace(place: string, disabledPlaces: string[]): string {
    if (place === "top") {
      if (!disabledPlaces.includes("top")) {
        if (
          topDistance < tooltipCardHeight.value + (props.arrow ? 4 : 6) &&
          ...
          if (
            ...
            rightDistance >= horizontalDistance
          ) {
            return "top";
          } else {
      ...
      if (!disabledPlaces.includes("bottom")) {
        if (
          bottomDistance < tooltipCardHeight.value + (props.arrow ? 4 : 6) &&
          disabledPlaces.length !== 3
        ) {
      ...
      if (!disabledPlaces.includes("left")) {
        if (
          leftDistance < tooltipCardWidth.value + (props.arrow ? 4 : 6) &&
          disabledPlaces.length !== 3
        ) {
          return findPlace("right", [...disabledPlaces, "left"]);
        } else {
          if (
            topDistance >= verticalDistance &&
            bottomDistance >= verticalDistance
          ) {
            return "left";
          } else {
            if (disabledPlaces.length !== 3) {
      ...
      if (!disabledPlaces.includes("right")) {
        if (
          rightDistance < tooltipCardWidth.value + (props.arrow ? 4 : 6) &&
          disabledPlaces.length !== 3
        ) {
  ...
  >
    ...
    >
      <div
        ...
        @keydown.esc="
          trigger === 'click' && keyboard && tooltipShow
            ? onHide()
            : () => false
        "
      >
    ...
    <span
      ...
      :style="contentStyle"
      @click="
        showTooltip && trigger === 'click' && !tooltipShow
          ? onShow()
          : () => false
      "
      @keydown.enter="
        showTooltip && trigger === 'click' && keyboard
          ? toggleVisible()
          : () => false
      "
      @keydown.esc="
        showTooltip && trigger === 'click' && keyboard && tooltipShow
          ? onHide()
          : () => false
      "
    >
...
.m-tooltip-wrap .m-tooltip-card .tooltip-card {
  min-width: 32px;
...
.m-tooltip-wrap .m-tooltip-card svg {
  fill: currentColor;
}

.m-tooltip-wrap .m-tooltip-card .tooltip-arrow {
  position: absolute;
  z-index: 9;
  display: block;
  pointer-events: none;
...
.m-tooltip-wrap .m-tooltip-card .tooltip-arrow::before {
  position: absolute;
  width: 16px;
  height: 8px;
  background-color: var(--tooltip-background-color);
...
.m-tooltip-wrap .m-tooltip-card .tooltip-arrow::after {
  position: absolute;
...
.m-tooltip-wrap .m-tooltip-card .arrow-top {
  left: 50%;
  bottom: 12px;
  transform: translateX(-50%) translateY(100%) rotate(180deg);
}

.m-tooltip-wrap .m-tooltip-card .arrow-top::before {
  bottom: 0;
  left: 0;
}

.m-tooltip-wrap .m-tooltip-card .arrow-top::after {
  bottom: 0;
  left: 0;
  right: 0;
}

.m-tooltip-wrap .m-tooltip-card .arrow-bottom {
  left: 50%;
  top: 12px;
  transform: translateX(-50%) translateY(-100%) rotate(0deg);
}

.m-tooltip-wrap .m-tooltip-card .arrow-bottom::before {
  bottom: 0;
  left: 0;
}

.m-tooltip-wrap .m-tooltip-card .arrow-bottom::after {
  bottom: 0;
  left: 0;
  right: 0;
}

.m-tooltip-wrap .m-tooltip-card .arrow-left {
  top: 50%;
  right: 12px;
  transform: translateX(100%) translateY(-50%) rotate(90deg);
}

.m-tooltip-wrap .m-tooltip-card .arrow-left::before {
  bottom: 0;
  left: 0;
}

.m-tooltip-wrap .m-tooltip-card .arrow-left::after {
  bottom: 0;
  left: 0;
  right: 0;
}

.m-tooltip-wrap .m-tooltip-card .arrow-right {
  top: 50%;
  left: 12px;
  transform: translateX(-100%) translateY(-50%) rotate(-90deg);
}

.m-tooltip-wrap .m-tooltip-card .arrow-right::before {
  bottom: 0;
  left: 0;
}

.m-tooltip-wrap .m-tooltip-card .arrow-right::after {
  bottom: 0;
  left: 0;
  right: 0;
}

.m-tooltip-wrap .tooltip-top-padding {
  padding-bottom: 12px;
}

.m-tooltip-wrap .tooltip-bottom-padding {
  padding-top: 12px;
}

.m-tooltip-wrap .tooltip-left-padding {
  padding-right: 12px;
}

.m-tooltip-wrap .tooltip-right-padding {
  padding-left: 12px;
}

.m-tooltip-wrap .tooltip-content {
  display: inline-block;
}
</style>
